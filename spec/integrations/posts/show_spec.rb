...
RSpec.describe 'Post show', type: :feature do
  ...
  it 'display post\'s title' do
    ...
    expect(page).to have_content "#{@post1.title} by #{@user1.name}"
  end
  ...
  it 'display post comments count' do
    visit user_post_path(@user2, @post2)
    expect(page).to have_content("Number of Comments: #{@post2.comments_counter}")
  end

  it 'display post likes count' do
    visit user_post_path(@user2, @post2)
  ...
  it 'display post commenter\'s name' do
    visit user_post_path(@user1, @post1)
  ...
  it 'display post commenter\'s text' do
    visit user_post_path(@user2, @post2)
